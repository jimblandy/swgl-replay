//! A `Serializer` that saves SWGL method calls in files.
...
impl gl_replay::Serializer for Files {
    type Error = io::Error;

    fn write_call(&mut self, call: &Call) -> Result<(), Self::Error> {
        self.calls.write_all(gl_replay::raw::as_bytes(call))?;
        Ok(())
    }
...
/// A recording of SWGL calls, created with `Recorder`.
